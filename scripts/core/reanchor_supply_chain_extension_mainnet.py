#!/usr/bin/env python3
"""
Anchor AEM V1.1 Supply-Chain Verification Extension manifest on Ethereum mainnet.

Safety controls:
  - DRY_RUN=1 prints payload and tx plan, never broadcasts.
  - BROADCAST=1 is required for a real broadcast.
  - Canonical manifest SHA-256 must match the official expected hash.
...
Env vars:
  ...
  ETHICBIT_SUPPLY_CHAIN_EXPECTED_SHA256=<hex64>       # optional override
"""
...
MANIFEST_HASH_RECORD_PATH = (
    ROOT / "assurance" / "release" / "AEM_V1_1_SUPPLY_CHAIN_VERIFICATION_HASH_RECORD.txt"
)

EXT_RECEIPT_PATH = (
    ROOT / "receipts" / "AEM_V1_1_SUPPLY_CHAIN_VERIFICATION_EXTENSION_RECEIPT.json"
)
...
ANCHOR_RECEIPT_PATH = (
    ROOT
    / "receipts"
    / "AEM_V1_1_SUPPLY_CHAIN_VERIFICATION_EXTENSION_MAINNET_ANCHOR_RECEIPT.json"
)

DEFAULT_EXPECTED_CANONICAL_SHA = (
    "a03af5496d8c3fa89af5bf1b0d40e03741a89bbc23bb97fd37a287c22d43fcef"
)

SCHEMA_ID = "ETHICBIT_AEM_V1_1_SUPPLY_CHAIN_VERIFICATION_EXTENSION_MAINNET_ANCHOR_PAYLOAD_V1"
RECEIPT_SCHEMA_ID = (
    "ETHICBIT_AEM_V1_1_SUPPLY_CHAIN_VERIFICATION_EXTENSION_MAINNET_ANCHOR_RECEIPT_V1"
)
...
def _expected_canonical_hash() -> str:
    env_override = os.environ.get("ETHICBIT_SUPPLY_CHAIN_EXPECTED_SHA256", "").strip().lower()
    if env_override:
        return env_override

    if MANIFEST_HASH_RECORD_PATH.exists():
        text = MANIFEST_HASH_RECORD_PATH.read_text(encoding="utf-8")
        m = re.search(
...
def _build_payload(
    ...
    return {
        ...
        "extension_claim_level": "PUBLIC_REPRODUCIBLE_VERIFICATION_SUPPORT",
        "extension_manifest_canonical_sha256": manifest_canonical_sha256,
        "extension_manifest_file_sha256": manifest_file_sha256,
        "extension_manifest_canonicalization": "rfc8785_jcs",
        "extension_manifest_hash_algorithm": "SHA-256",
        "extension_manifest_relative_path": str(MANIFEST_JSON_PATH.relative_to(ROOT)),
        "extension_manifest_jcs_relative_path": str(MANIFEST_JCS_PATH.relative_to(ROOT)),
...
def main() -> int:
    ...
    if missing:
        print("ERROR: missing required extension inputs:", file=sys.stderr)
        for item in missing:
            print(f" - {item}", file=sys.stderr)
        return 1

    manifest_canonical_sha = _sha256_file(MANIFEST_JCS_PATH)
    manifest_file_sha = _sha256_file(MANIFEST_JSON_PATH)
    expected_sha = _expected_canonical_hash()

    print("=== SUPPLY-CHAIN MANIFEST HASH CHECK ===")
    print(f"manifest_json_path = {MANIFEST_JSON_PATH}")
    print(f"manifest_jcs_path  = {MANIFEST_JCS_PATH}")
    ...
    print("NOTE: canonical SHA-256 is computed over manifest .jcs.json bytes.")

    if manifest_canonical_sha.lower() != expected_sha.lower():
        print(
            "ERROR: extension manifest canonical hash does not match expected official hash.",
            file=sys.stderr,
        )
        return 1

    extension_receipt = json.loads(EXT_RECEIPT_PATH.read_text(encoding="utf-8"))
    registry_receipt = json.loads(REGISTRY_RECEIPT_PATH.read_text(encoding="utf-8"))
    ...
    if dry_run:
        ...
        if ANCHOR_RECEIPT_PATH.exists():
            try:
                existing = json.loads(ANCHOR_RECEIPT_PATH.read_text(encoding="utf-8"))
    ...
    anchor_receipt = {
        ...
        "status": (
            "ONCHAIN_SUPPLY_CHAIN_EXTENSION_ANCHOR_VERIFIED"
            if status == 1
            else "ONCHAIN_SUPPLY_CHAIN_EXTENSION_ANCHOR_FAILED"
        ),
        "network": NETWORK,
        "chain_id": CHAIN_ID,
        "tx_hash": tx_hash_hex,
        ...
        "extension_manifest": {
            "manifest_path": str(MANIFEST_JSON_PATH.relative_to(ROOT)),
            "manifest_jcs_path": str(MANIFEST_JCS_PATH.relative_to(ROOT)),
            "manifest_canonical_sha256": manifest_canonical_sha,
            "manifest_file_sha256": manifest_file_sha,
